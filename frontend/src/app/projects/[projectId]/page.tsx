...
import {
  createProjectMemoryApiV1ProjectsProjectIdMemoryPost,
  streamProjectMemoryApiV1ProjectsProjectIdMemoryStreamGet,
} from "@/api/generated/project-memory/project-memory";
import {
  type getMyMembershipApiV1OrganizationsMeMemberGetResponse,
  useGetMyMembershipApiV1OrganizationsMeMemberGet,
} from "@/api/generated/organizations/organizations";
import {
  createTaskApiV1ProjectsProjectIdTasksPost,
  createTaskCommentApiV1ProjectsProjectIdTasksTaskIdCommentsPost,
  deleteTaskApiV1ProjectsProjectIdTasksTaskIdDelete,
  listTaskCommentsApiV1ProjectsProjectIdTasksTaskIdCommentsGet,
  streamTasksApiV1ProjectsProjectIdTasksStreamGet,
  updateTaskApiV1ProjectsProjectIdTasksTaskIdPatch,
...
type BoardChatMessage = ProjectMemoryRead;
...
type BoardTaskCreatePayload = Parameters<
  typeof createTaskApiV1ProjectsProjectIdTasksPost
>[1] &
  TaskCustomFieldPayload;
type BoardTaskUpdatePayload = Parameters<
  typeof updateTaskApiV1ProjectsProjectIdTasksTaskIdPatch
>[2] &
  TaskCustomFieldPayload;
...
const resolveBoardAccess = (
  ...
  if (member.all_projects_write) {
    return { canRead: true, canWrite: true };
  }
  if (member.all_projects_read) {
    return { canRead: true, canWrite: false };
  }
  const entry = member.project_access?.find(
    (access) => access.project_id === projectId,
  );
  if (!entry) {
    return { canRead: false, canWrite: false };
  }
...
export default function BoardDetailPage() {
  ...
  const boardCustomFieldDefinitions = useMemo(() => {
    if (!projectId || customFieldDefinitionsQuery.data?.status !== 200) {
      return [] as TaskCustomFieldDefinitionRead[];
    }
    return (customFieldDefinitionsQuery.data.data ?? [])
      .filter((definition) => (definition.project_ids ?? []).includes(projectId))
      .sort((left, right) =>
        (left.label || left.field_key).localeCompare(
          right.label || right.field_key,
        ),
      );
  }, [projectId, customFieldDefinitionsQuery.data]);
  ...
  const boardAccess = useMemo(
    ...
    [membershipQuery.data, projectId],
  );
  ...
  const loadBoard = useCallback(async () => {
    ...
    try {
      const snapshotResult =
        await getProjectSnapshotApiV1ProjectsProjectIdSnapshotGet(projectId);
      if (snapshotResult.status !== 200) {
        throw new Error("Unable to load project snapshot.");
      }
      const snapshot = snapshotResult.data;
      setBoard(snapshot.project);
  ...
  useEffect(() => {
    ...
    if (!isSignedIn || !projectId || !board) return;
  ...
  }, [board, projectId, isPageActive, isSignedIn, pushLiveFeed]);
  ...
  useEffect(() => {
    ...
    const connect = async () => {
      try {
        ...
        const streamResult = await streamTasksApiV1ProjectsProjectIdTasksStreamGet(
          projectId,
          since ? { since } : undefined,
          {
            headers: { Accept: "text/event-stream" },
  ...
  useEffect(() => {
    if (!isPageActive) return;
    if (!isSignedIn || !projectId || !isOrgAdmin) return;
    let isCancelled = false;
    const abortController = new AbortController();
    const backoff = createExponentialBackoff(SSE_RECONNECT_BACKOFF);
    let reconnectTimeout: number | undefined;
  ...
  }, [board, projectId, isOrgAdmin, isPageActive, isSignedIn, pushLiveFeed]);
  ...
  const handleCreateTask = async () => {
    ...
    try {
      ...
      const result = await createTaskApiV1ProjectsProjectIdTasksPost(
        projectId,
        payload,
      );
  ...
  const postBoardChatMessage = useCallback(
    async (content: string): Promise<{ ok: boolean; error: string | null }> => {
      ...
      try {
        const result = await createProjectMemoryApiV1ProjectsProjectIdMemoryPost(
          projectId,
          {
            content: trimmed,
            tags: ["chat"],
    ...
    [projectId, currentUserDisplayName, isSignedIn, pushLiveFeed],
  );
  ...
  const assignableAgents = useMemo(
    () => agents.filter((agent) => !agent.is_project_lead),
    [agents],
  );
  ...
  const boardLead = useMemo(
    () => agents.find((agent) => agent.is_project_lead) ?? null,
    [agents],
  );
  ...
  const loadComments = useCallback(
    async (taskId: string) => {
      ...
      try {
        const result =
          await listTaskCommentsApiV1ProjectsProjectIdTasksTaskIdCommentsGet(
            projectId,
            taskId,
          );
  ...
  const handlePostComment = async (message: string): Promise<boolean> => {
    if (!selectedTask || !projectId || !isSignedIn) return false;
    const trimmed = message.trim();
    if (!trimmed) {
    ...
    try {
      const result =
        await createTaskCommentApiV1ProjectsProjectIdTasksTaskIdCommentsPost(
          projectId,
          selectedTask.id,
          { message: trimmed },
        );
  ...
  const handleTaskSave = async (closeOnSuccess = false) => {
    if (!selectedTask || !isSignedIn || !projectId) return;
    ...
    try {
      ...
      const result = await updateTaskApiV1ProjectsProjectIdTasksTaskIdPatch(
        projectId,
        selectedTask.id,
        updatePayload,
      );
  ...
  const handleDeleteTask = async () => {
    if (!selectedTask || !projectId || !isSignedIn) return;
    setIsDeletingTask(true);
    setDeleteTaskError(null);
    try {
      const result = await deleteTaskApiV1ProjectsProjectIdTasksTaskIdDelete(
        projectId,
        selectedTask.id,
      );
  ...
  const handleTaskMove = useCallback(
    ...
    [projectId, isSignedIn, pushToast, taskTitleById],
  );
  ...
  const agentAvatarLabel = (agent: Agent) => {
    if (agent.is_project_lead) return "⚙️";
  ...
  const agentRoleLabel = (agent: Agent) => {
    ...
    if (agent.is_project_lead) return "Project lead";
  ...
  const handleApprovalDecision = useCallback(
    async (approvalId: string, status: "approved" | "rejected") => {
      ...
      try {
        const result =
          await updateApprovalApiV1ProjectsProjectIdApprovalsApprovalIdPatch(
            projectId,
            approvalId,
            { status },
          );
    ...
    [projectId, canWrite, isSignedIn, pushToast],
  );
