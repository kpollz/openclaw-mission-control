...
import {
  ...
  getOnboardingApiV1ProjectsProjectIdOnboardingGet,
  startOnboardingApiV1ProjectsProjectIdOnboardingStartPost,
} from "@/api/generated/project-onboarding/project-onboarding";
import type {
  ProjectOnboardingAgentComplete,
  ProjectOnboardingRead,
  ProjectOnboardingReadMessages,
  ProjectRead,
...
const normalizeMessages = (
  value?: ProjectOnboardingReadMessages,
...
export function ProjectOnboardingChat({
  projectId,
  onConfirmed,
}: {
  projectId: string;
  onConfirmed: (project: ProjectRead) => void;
}) {
  const isPageActive = usePageActive();
  const [session, setSession] = useState<ProjectOnboardingRead | null>(null);
  ...
  const draft: ProjectOnboardingAgentComplete | null =
  ...
  const startSession = useCallback(async () => {
    ...
    setError(null);
    try {
      const result = await startOnboardingApiV1ProjectsProjectIdOnboardingStartPost(
        projectId,
        {},
      );
  ...
  const refreshSession = useCallback(async () => {
    try {
      const result =
        await getOnboardingApiV1ProjectsProjectIdOnboardingGet(projectId);
  ...
  const handleAnswer = useCallback(
    async (value: string, freeText?: string) => {
      ...
      try {
        const result =
          await answerOnboardingApiV1ProjectsProjectIdOnboardingAnswerPost(
            projectId,
            {
              answer: value,
    ...
    [projectId, lastAssistantFingerprint],
  );
  ...
  const submitExtraContext = useCallback(async () => {
    ...
    try {
      const result =
        await answerOnboardingApiV1ProjectsProjectIdOnboardingAnswerPost(projectId, {
  ...
  }, [projectId, extraContext, lastAssistantFingerprint]);
  ...
  const confirmGoal = async () => {
    ...
    try {
      const result =
        await confirmOnboardingApiV1ProjectsProjectIdOnboardingConfirmPost(
          projectId,
          {
            project_type: draft.project_type ?? "goal",
      ...
      {draft ? (
        <div className="space-y-3">
          ...
          <div className="rounded-lg border border-slate-200 bg-slate-50 p-3 text-sm">
            ...
            <p className="text-slate-700">{draft.project_type || "goal"}</p>
            {draft.user_profile ? (
              <>
