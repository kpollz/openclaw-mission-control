...
from app.presentation.schemas.gateways import GatewayTemplatesSyncResult
from app.infrastructure.gateway.constants import DEFAULT_HEARTBEAT_CONFIG
...
class GatewayAdminLifecycleService(OpenClawDBService):
    ...
    async def clear_agent_foreign_keys(self, *, agent_id: UUID) -> None:
        ...
        await crud.update_where(
            self.session,
            ProjectWebhook,
            col(ProjectWebhook.agent_id) == agent_id,
            agent_id=None,
            updated_at=now,
            commit=False,
        )
