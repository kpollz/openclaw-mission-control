...
from app.infrastructure.gateway.constants import CHECKIN_DEADLINE_AFTER_WAKE
...
from app.application.use_cases.organizations.service import get_org_owner_user
...
class AgentLifecycleOrchestrator(OpenClawDBService):
    ...
    async def run_lifecycle(
        ...
        *,
        ...
        template_user = user
        if project is None and template_user is None:
        ...
        pre_mint_token_hash = locked.agent_token_hash
        ...
        if not gateway.url:
            ...
            locked.last_provision_error = "Gateway URL is not configured."
        ...
        try:
            ...
            locked.agent_token_hash = pre_mint_token_hash
            locked.status = previous_status
            locked.checkin_deadline_at = None
