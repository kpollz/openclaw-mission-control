...
async def _query_throughput(
    ...
    statement = (
        statement.where(col(Task.project_id).in_(project_ids)).group_by(bucket_col).order_by(bucket_col)
    )
...
async def _query_wip(
    ...
    inbox_statement = (
        ...
        .where(col(Task.project_id).in_(project_ids))
...
async def _active_agents(
    ...
    if not project_ids:
        return 0
    statement = statement.where(col(Agent.project_id).in_(project_ids))
...
async def _pending_approvals_snapshot(
    ...
    *,
    ...
    total_statement = (
        ...
        .where(col(Approval.project_id).in_(project_ids))
    ...
    rows = (
        await session.exec(
            select(Approval, Project, Task)
            .join(Project, col(Project.id) == col(Approval.project_id))
    ...
    items = [
        ...
        for approval, project, task in rows
    ]
    return DashboardPendingApprovals(total=total, items=items)


async def _resolve_dashboard_project_ids(
    session: AsyncSession,
    *,
    ...
    return [project_id] if project_id is not None else project_ids


class MetricsService:
    """Facade for dashboard metric aggregation."""
    ...
    async def dashboard_metrics(
        ...
        *,
        ...
        project_ids = await _resolve_dashboard_project_ids(
            session,
            ctx=ctx,
            project_id=project_id,
        )

        throughput_primary = await _query_throughput(session, primary, project_ids)
        throughput_comparison = await _query_throughput(session, comparison, project_ids)
        throughput = DashboardSeriesSet(
            primary=throughput_primary,
            comparison=throughput_comparison,
        )
        cycle_time_primary = await _query_cycle_time(session, primary, project_ids)
        cycle_time_comparison = await _query_cycle_time(session, comparison, project_ids)
        cycle_time = DashboardSeriesSet(
            primary=cycle_time_primary,
            comparison=cycle_time_comparison,
        )
        error_rate_primary = await _query_error_rate(session, primary, project_ids)
        error_rate_comparison = await _query_error_rate(session, comparison, project_ids)
        error_rate = DashboardSeriesSet(
            primary=error_rate_primary,
            comparison=error_rate_comparison,
        )
        wip_primary = await _query_wip(session, primary, project_ids)
        wip_comparison = await _query_wip(session, comparison, project_ids)
        wip = DashboardWipSeriesSet(
            primary=wip_primary,
            comparison=wip_comparison,
        )
        task_status_counts = await _task_status_counts(session, project_ids)
        pending_approvals = await _pending_approvals_snapshot(session, project_ids, limit=10)

        kpis = DashboardKpis(
            active_agents=await _active_agents(session, primary, project_ids),
            ...
            error_rate_pct=await _error_rate_kpi(session, primary, project_ids),
