"""Helpers for assembling denormalized project snapshot response payloads."""
...
from app.presentation.schemas.view_models import ProjectSnapshot, TaskCardRead
from app.infrastructure.persistence.approval_task_links import load_task_ids_by_approval, task_counts_for_project
from app.application.use_cases.agents.provisioning_db import AgentLifecycleService
from app.application.use_cases.tags.service import TagState, load_tag_state
...
def _memory_to_read(memory: ProjectMemory) -> ProjectMemoryRead:
    return ProjectMemoryRead.model_validate(memory, from_attributes=True)
...
async def build_project_snapshot(session: AsyncSession, project: Project) -> ProjectSnapshot:
    """Build a project snapshot with tasks, agents, approvals, and chat history."""
    project_read = ProjectRead.model_validate(project, from_attributes=True)

    tasks = list(
        await Task.objects.filter_by(project_id=project.id)
    ...
    pending_approvals_count = int(
        (
            await session.exec(
                ...
                .where(col(Approval.project_id) == project.id)
    ...
    approvals = (
        await Approval.objects.filter_by(project_id=project.id)
    ...
    counts_by_task_id = await task_counts_for_project(session, project_id=project.id)
    ...
    chat_messages = (
        await ProjectMemory.objects.filter_by(project_id=project.id)
        .filter(col(ProjectMemory.is_chat).is_(True))
        ...
        .order_by(col(ProjectMemory.created_at).desc())
        .limit(200)
        .all(session)
    )
