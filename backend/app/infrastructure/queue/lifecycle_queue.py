...
from app.infrastructure.queue.redis_queue import QueuedTask, enqueue_task_with_delay
from app.infrastructure.queue.redis_queue import requeue_if_failed as generic_requeue_if_failed
...
def decode_lifecycle_task(task: QueuedTask) -> QueuedAgentLifecycleReconcile:
    ...
    raw_project_id = payload.get("project_id") or payload.get("project_id")
    project_id = UUID(raw_project_id) if isinstance(raw_project_id, str) and raw_project_id else None
