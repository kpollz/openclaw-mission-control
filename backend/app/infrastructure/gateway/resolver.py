...
Goals:
- Centralize "project -> gateway row" resolution and defensive org checks.
...
async def get_gateway_for_project(
    session: AsyncSession,
    project: Project,
) -> Gateway | None:
    """Return the gateway for a project when present and valid; otherwise return None."""
    if project.gateway_id is None:
        return None
    gateway = await Gateway.objects.by_id(project.gateway_id).first(session)
    if gateway is None:
        return None
    # Defensive guard: projects and gateways are tenant-scoped; reject cross-org mismatches.
    if gateway.organization_id != project.organization_id:
        return None
    return gateway


async def require_gateway_for_project(
    session: AsyncSession,
    project: Project,
    *,
    require_workspace_root: bool = False,
) -> Gateway:
    """Return a project's gateway or raise a 422 with a stable error message."""
    if project.gateway_id is None:
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_CONTENT,
    ...
    if gateway is None:
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_CONTENT,
            detail="Project gateway_id is invalid",
        )
