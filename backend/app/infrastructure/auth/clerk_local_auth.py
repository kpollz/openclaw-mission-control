"""User authentication helpers for Clerk, local-token, and password auth modes.
...
- `password`: JWT tokens issued by register/login endpoints for self-hosted
...
async def _resolve_password_auth_context(
    *,
    request: Request,
    session: AsyncSession,
    required: bool,
) -> AuthContext | None:
    """Resolve user from a JWT access token issued by /auth/register or /auth/login."""
    token = _extract_bearer_token(request.headers.get("Authorization"))
    if token is None:
        if required:
            raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED)
    ...
    user = (await session.exec(select(User).where(col(User.id) == user_id))).first()
    if user is None:
        if required:
            raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED)
        return None
...
async def get_auth_context(
    ...
    if settings.auth_mode == AuthMode.PASSWORD:
        pw_auth = await _resolve_password_auth_context(
            request=request,
            session=session,
            required=True,
        )
        if pw_auth is None:  # pragma: no cover
            raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED)
        return pw_auth
...
async def get_auth_context_optional(
    ...
    if settings.auth_mode == AuthMode.PASSWORD:
        return await _resolve_password_auth_context(
            request=request,
            session=session,
            required=False,
        )
