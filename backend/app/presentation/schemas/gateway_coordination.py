...
class GatewayLeadMessageResponse(SQLModel):
    ...
    project_id: UUID = Field(description="Project receiving the message.")
...
class GatewayLeadBroadcastRequest(SQLModel):
    ...
    content: NonEmptyStr = Field(
        ...
        examples=["Project-wide incident: prioritize risk triage on task set 14."],
    )
...
class GatewayLeadBroadcastProjectResult(SQLModel):
    ...
    project_id: UUID = Field(description="Target board id for this result.")
    lead_agent_id: UUID | None = Field(
        default=None,
...
class GatewayLeadBroadcastResponse(SQLModel):
    ...
    model_config = SQLModelConfig(
        json_schema_extra={
            ...
            "x-response-shape": "List of GatewayLeadBroadcastProjectResult",
        },
    )
    ...
    results: list[GatewayLeadBroadcastProjectResult] = Field(default_factory=list)
...
class GatewayMainAskUserResponse(SQLModel):
    ...
    project_id: UUID = Field(description="Project context used for the request.")
