"""Schemas for task custom field metadata, project bindings, and payloads."""
...
class TaskCustomFieldDefinitionCreate(TaskCustomFieldDefinitionBase):
    ...
    @classmethod
    def normalize_project_ids(cls, value: list[UUID]) -> list[UUID]:
        """Remove duplicates while preserving user-supplied order."""
        deduped = list(dict.fromkeys(value))
        if not deduped:
            raise ValueError("project_ids must include at least one project")
        return deduped
...
class TaskCustomFieldDefinitionUpdate(SQLModel):
    ...
    @field_validator("project_ids")
    @classmethod
    def normalize_project_ids(cls, value: list[UUID] | None) -> list[UUID] | None:
        """Normalize project bindings when provided in updates."""
        if value is None:
            return None
        deduped = list(dict.fromkeys(value))
        if not deduped:
...
class ProjectTaskCustomFieldCreate(SQLModel):
    """Payload for binding a definition to a project."""

    task_custom_field_definition_id: UUID


class ProjectTaskCustomFieldRead(SQLModel):
    """Payload returned when listing project-bound custom fields."""
