"""Schemas used by the project-onboarding assistant flow."""
...
class ProjectOnboardingStart(SQLModel):
    """Start signal for initializing onboarding conversation."""


class ProjectOnboardingAnswer(SQLModel):
...
class ProjectOnboardingConfirm(SQLModel):
    ...
    def validate_goal_fields(self) -> Self:
        """Require goal metadata when the project type is `goal`."""
        if self.project_type == "goal" and (not self.objective or not self.success_metrics):
            message = "Confirmed goal projects require objective and success_metrics"
            raise ValueError(message)
        return self


class ProjectOnboardingQuestionOption(SQLModel):
...
class ProjectOnboardingAgentQuestion(SQLModel):
    """Question payload emitted by the onboarding assistant."""

    question: NonEmptyStr
    options: list[ProjectOnboardingQuestionOption] = Field(min_length=1)
...
class ProjectOnboardingUserProfile(SQLModel):
...
class ProjectOnboardingLeadAgentDraft(SQLModel):
...
class ProjectOnboardingAgentComplete(ProjectOnboardingConfirm):
    """Complete onboarding draft produced by the onboarding assistant."""

    status: Literal["complete"]
    user_profile: ProjectOnboardingUserProfile | None = None
    lead_agent: ProjectOnboardingLeadAgentDraft | None = None


ProjectOnboardingAgentUpdate = ProjectOnboardingAgentComplete | ProjectOnboardingAgentQuestion


class ProjectOnboardingRead(SQLModel):
    ...
    draft_goal: ProjectOnboardingAgentComplete | None = None
    created_at: datetime
    updated_at: datetime
