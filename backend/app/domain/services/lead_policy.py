...
def task_fingerprint(title: str, description: str | None, project_id: str) -> str:
    """Build a stable hash key for deduplicating similar project tasks."""
    normalized_title = title.strip().lower()
    normalized_desc = (description or "").strip().lower()
    seed = f"{project_id}::{normalized_title}::{normalized_desc}"
